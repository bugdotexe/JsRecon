...
# ---------------------------------------------------------------------
#  Find and (optionally) Download Source Maps
# ---------------------------------------------------------------------

def find_source_maps(js_dir: Path, output_file: Path, download=False):
    ...
    if download and maps:
        print("[*] Downloading source map files ...")
        download_js_files(output_file, js_dir / "_maps", jobs=8)
...
def main():
    ...
    parser.add_argument("--dlmaps", action="store_true", help="Also download the .map files found")
    ...
    if args.download:
        ...
        # Phase 3: Find (and optionally download) source maps
        if args.findmaps:
            find_source_maps(args.download, args.output.with_suffix(".sourcemaps.txt"), download=args.dlmaps)
